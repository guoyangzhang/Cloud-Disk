export default {
	...
	/**
	 * 处理应用跳转的url
	 * appUrl应用地址 paramMap参数
	 */
	dealAppUrl (appUrl, paramMap) {
		let result = appUrl
		if (paramMap.length > 0) {
			...
			if (paramMap.length === 1) {
				paramMap.map(function (item, index, paramMap) {
					if (item.idCard && item.name) {
						result += (queryStr + item.idCard + equalStr + item.name)
					}
				})
			} else {
				paramMap.map(function (item, index, paramMap) {
					if (index === 0) {
						...
						if (item.idCard && item.name) {
							result += (concatStr + item.idCard + equalStr + item.name)
						}
					}
				})
			}
		}
		if (!(result.indexOf('https://') === 0 || result.indexOf('http://') === 0)) {
			result = 'http://' + result
		}
		return result
	},
